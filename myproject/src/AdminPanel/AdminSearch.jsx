...
export default function Searchbar() {
  ...
  const getdata = () => {
    ...
    axios
      .get(`https://burgundy-cow-kit.cyclic.app//GirlKids `)
      .then((res) => setdata([...data, ...res.data]));
    axios
      .get(`https://burgundy-cow-kit.cyclic.app///Footwear  `)
      .then((res) => setdata([...data, ...res.data]));
  };
  useEffect(() => {
    getdata();
  ...
  const handleactive = (e) => {
    ...
    if (e.keyCode === 13) {
      setfocus(false);
      dispatch(getProducts({ q: input })).then(() => {
        navigate("/products");
      });
    }
  };
  ...
  const handleclicksuggestion = () => {
    dispatch(getProducts({ q: suggestions[activeoption].trim() })).then(() => {
      navigate("/products");
    });
  };
  return (
